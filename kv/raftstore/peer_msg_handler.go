...
func (d *peerMsgHandler) searchPeer(nodeId uint64) int {
...
func (d *peerMsgHandler) processConfChange(entry *pb.Entry, cc *pb.ConfChange, kvWB *engine_util.WriteBatch) *engine_util.WriteBatch {
	...
	case eraftpb.ConfChangeType_AddNode:
		...
		if d.searchPeer(cc.NodeId) == len(thisRegion.Peers) {
			...
			storeMeta.Lock()
			// region 中追加新的 peer
			thisRegion.Peers = append(thisRegion.Peers, req.Peer)
			//更改版本信息！
			thisRegion.RegionEpoch.ConfVer++
	...
	case eraftpb.ConfChangeType_RemoveNode:
		...
		if NodeI != len(thisRegion.Peers) { //目标peer存在于region
			storeMeta := d.ctx.storeMeta
			storeMeta.Lock()

			thisRegion.Peers = append(thisRegion.Peers[:NodeI], thisRegion.Peers[NodeI+1:]...)
			thisRegion.RegionEpoch.ConfVer++
			meta.WriteRegionState(kvWB, thisRegion, rspb.PeerState_Normal)
...
func (d *peerMsgHandler) processAdminRequest(entry *pb.Entry, request *raft_cmdpb.RaftCmdRequest, kvWB *engine_util.WriteBatch) *engine_util.WriteBatch {
	...
	case raft_cmdpb.AdminCmdType_Split:
		...
		err = util.CheckKeyInRegion(splitReq.SplitKey, d.Region()) //has key
		if err != nil {
			d.handleProposal(entry, ErrResp(err))
			return kvWB
		}
		if len(splitReq.NewPeerIds) != len(d.Region().Peers) { //个数不对
			d.handleProposal(entry, ErrResp(errors.Errorf("length of NewPeerIds != Peers")))
			return kvWB
		}
		newPeersForMeta := []*metapb.Peer{}
		for i, p := range d.Region().Peers {
			newPeersForMeta = append(newPeersForMeta, &metapb.Peer{
		...
		err = d.ctx.router.send(splitReq.NewRegionId, message.Msg{
